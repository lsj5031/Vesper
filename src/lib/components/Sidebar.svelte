...
async function handleDeleteFeed(feed: Feed, e: Event) {
    ...
    const confirmed = await showConfirm({
        title: "Delete Feed?",
        message: `Are you sure you want to delete "${feed.title}"?`,
        confirmText: "Delete",
        cancelText: "Cancel",
        isDangerous: true,
    });
...
>
    ...
    <div class="p-4 border-b border-o3-black-30">
        <h1
            class={`text-2xl font-headline font-bold tracking-tight mb-1 ${$themeMode === "dark" ? "text-o3-white" : "text-o3-black-90"}`}
        >
            Vesper
        </h1>
        <p
            class={`text-xs uppercase tracking-widest ${$themeMode === "dark" ? "text-o3-black-50" : "text-o3-black-70"}`}
        >
    ...
    <div
        class="p-4 space-y-2"
        style={`border-bottom: 1px solid ${$themeMode === "dark" ? "var(--o3-color-palette-black-30)" : "var(--o3-color-palette-black-10)"}`}
    >
        <!-- Search -->
        ...
        <div class="flex gap-1">
            {#if $refreshProgress}
                ...
                >
                    <div
                        class="absolute inset-0 bg-o3-teal opacity-30 transition-all duration-500 ease-out"
                        style="width: {($refreshProgress.completed / $refreshProgress.total) *
                            100}%"
                    ></div>
                    <span class="relative z-10"
                        >Updating {$refreshProgress.completed}/{$refreshProgress.total}</span
                    >
                </button>
            {:else}
                <button
            ...
            {/if}
            <button
                class="o3-button o3-button--secondary o3-button--small {isManaging
                    ? 'o3-button--primary'
                    : ''}"
                data-o3-theme={$themeMode === "dark" ? "inverse" : "standard"}
    ...
    <div class="flex-1 overflow-y-auto p-2 space-y-4">
        ...
        {#each foldersList as folder}
            <div class="space-y-0.5">
                ...
                {#each feedsList.filter((f) => f.folderId === folder.id) as feed}
                    ...
                    {@const hoverBg =
                        $themeMode === "dark" ? "hover:bg-o3-black-80/30" : "hover:bg-o3-black-10"}
                    {@const loadingBg =
                        $themeMode === "dark" ? "bg-o3-black-80/50" : "bg-o3-black-5"}
                    <div
                        class="group flex items-center w-full transition-all duration-200 ease-out {$selectedFeedId ===
                        feed.id
                            ? activeBg +
                              ' border-l-2 ' +
                              (feed.error ? 'border-o3-claret' : 'border-o3-teal')
                            : feed.error
                              ? activeBg + ' border-l-2 border-o3-claret'
                              : isUpdating
                                ? loadingBg
                                : hoverBg} hover:translate-x-1"
                    >
                        <button
                        ...
                        <div class="flex items-center pr-2 gap-1">
                            {#if feed.error}
                                <span
                                    class="text-o3-claret text-xs font-bold cursor-help"
                                    title={feed.error}>!</span
                                >
                            {/if}
        ...
        <div class="space-y-0.5">
            <div
                ...
                class:text-o3-black-50={$themeMode === "dark"}
            ...
            {#each feedsList.filter((f) => !f.folderId) as feed}
                ...
                >
                    <button
                        class="flex-1 text-left px-3 py-1.5 text-sm flex justify-between items-center gap-2 min-w-0 {$selectedFeedId ===
                        feed.id
                            ? $themeMode === 'dark'
                                ? 'text-o3-white'
                                : 'text-o3-black-90'
                            : feed.error
                              ? 'text-o3-claret'
                              : $themeMode === 'dark'
                                ? 'text-o3-black-30 group-hover:text-o3-white'
                                : 'text-o3-black-80 group-hover:text-o3-black-90'}"
                        on:click={() => selectFeed(feed.id)}
                    >
                        <span class="truncate">{feed.title}</span>
                        <span class="flex items-center gap-1 shrink-0">
