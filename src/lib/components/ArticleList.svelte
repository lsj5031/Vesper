...
function toggleSelectAll() {
    ...
    if (selectedVisibleCount === articles.length) {
        ...
        selectedIds = new Set(
            articles.map((a) => a.id).filter((id): id is number => id !== undefined)
        );
    }
}
    ...
    if (e.key === "j" || e.key === "k" || e.key === "ArrowDown" || e.key === "ArrowUp") {
        ...
        if (newIndex >= 0 && newIndex < articles.length) {
            ...
            if (newArticleId !== undefined) {
                ...
                // Smart alignment: when going down, position at top; when going up, position at bottom
                const align = e.key === "j" || e.key === "ArrowDown" ? "start" : "end";
                scrollToArticle(newArticleId, align);
            }
        }
        e.preventDefault();
    }
...
>
    {#if currentFeed?.error}
        <div
            class="bg-o3-claret text-white px-4 py-3 flex flex-col gap-2 shrink-0 border-b"
            class:border-o3-black-80={isDark}
            class:border-o3-black-30={!isDark}
        >
    ...
    <!-- Toolbar -->
    <div
        class="px-4 py-3 relative z-10 shrink-0"
        style={`background:${isDark ? "var(--o3-color-palette-black-90)" : "var(--o3-color-palette-paper)"};border-bottom: 1px solid ${isDark ? "var(--o3-color-palette-black-80)" : "var(--o3-color-palette-black-20)"}`}
    >
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex items-center gap-3 flex-wrap justify-end">
                <!-- Filter Buttons -->
                <div
                    class="flex rounded-full overflow-hidden text-xs font-semibold"
                    class:border={isDark}
                    class:border-o3-black-60={isDark}
                    style={isDark
                        ? "background: rgba(0, 0, 0, 0.25)"
                        : "background: rgb(255, 255, 255); border: 1px solid var(--o3-color-palette-black-10)"}
                >
                    <button
                    ...
                    <div
                        class="w-px"
                        class:bg-o3-black-60={isDark}
                        style={!isDark ? "background: var(--o3-color-palette-black-10)" : ""}
                    ></div>
                    <button
                        class="px-4 py-1.5 uppercase tracking-wide transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-o3-teal"
                ...
                <div class="flex items-center gap-2">
                    <button
                        ...
                        style={!selectionMode
                            ? isDark
                                ? "border-opacity: 0.4; background: rgba(17, 153, 142, 0.05)"
                                : "border-opacity: 0.3; background: rgba(17, 153, 142, 0.03)"
                            : ""}
                        on:click={() => {
                    ...
                    <div id="article-actions-menu" class="relative">
                        ...
                        {#if showActionMenu}
                            <div
                                class="absolute right-0 mt-2 w-52 border rounded overflow-hidden shadow-lg z-20"
                                class:bg-o3-black-80={isDark}
                                class:border-o3-black-40={isDark}
                                class:bg-o3-white={!isDark}
                                class:border-o3-black-10={!isDark}
                            >
                                <button
    ...
    {#if selectionMode}
        <div
            ...
            style={`background:${isDark ? "var(--o3-color-palette-black-90)" : "var(--o3-color-palette-paper)"};border-bottom: 1px solid ${isDark ? "var(--o3-color-palette-black-80)" : "var(--o3-color-palette-black-10)"}`}
        >
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <!-- Selection Info -->
                <div class="flex items-baseline gap-3">
                    <span
                        class="text-2xl font-headline font-bold leading-none"
                        class:text-o3-paper={isDark}
                    ...
                    <span
                        ...
                        class:text-o3-black-50={isDark}
                        class:text-o3-black-60={!isDark}
                    >
    ...
    <div
        id="article-list-scroll-container"
        class="flex-1 overflow-y-auto overflow-x-hidden min-h-0 relative"
    ...
    >
        {#if $articlesStore}
            {#each $articlesStore as article (article.id)}
                <div
                    ...
                    class:bg-o3-black-10={$selectedArticleId === article.id &&
                        $themeMode !== "dark"}
                    class:hover:bg-o3-black-80={$selectedArticleId !== article.id &&
                        $themeMode === "dark"}
                    class:hover:bg-o3-black-10={$selectedArticleId !== article.id &&
                        $themeMode !== "dark"}
                ...
                >
                    ...
                    >
                        ...
                        <h3
                            class="font-headline font-bold text-lg leading-snug mb-2"
                            class:text-o3-paper={$themeMode === "dark"}
                            class:text-o3-black-90={$themeMode !== "dark"}
                            class:group-hover:text-o3-white={$themeMode === "dark"}
                        >
                            {article.title}
                        </h3>

                        <div
                            class="text-sm line-clamp-2 mb-2 font-body leading-relaxed"
                            class:text-o3-black-20={$themeMode === "dark"}
            ...
            {#if $articlesStore.length === 0}
                <div class="p-8 text-center text-o3-black-40 italic">No articles found.</div>
            {/if}
        {:else}
