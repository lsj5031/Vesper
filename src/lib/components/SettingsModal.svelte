<script lang="ts">
    ...
    async function handleFileSelect(e: Event) {
         ...
         if (input.files?.length) {
            ...
            try {
                await importOPML(file, (progress: { current: number; total: number; currentFile: string }) => {
                    importProgress = {
    ...
    async function handleBackupSelect(e: Event) {
         ...
         if (input.files?.length) {
            ...
            importProgress = { current: 0, total: 0, stage: 'Reading file...', currentFile: '' };
            try {
                await importBackup(input.files[0], (progress: { stage: string; current: number; total: number }) => {
                    importProgress = {
                        current: progress.current,
                        total: progress.total,
...
<div class="fixed inset-0 z-50 flex items-center justify-center" style={`background:${isDark ? 'var(--o3-color-palette-black-90)' : 'var(--o3-color-palette-paper)'}`}>
    ...
    >
        ...
        {#if isImporting}
            <div class="mb-4 rounded border border-o3-teal px-4 py-3" style={`background:${isDark ? 'rgba(17, 153, 142, 0.12)' : 'rgba(17, 153, 142, 0.05)'}`}>
                ...
                {#if importProgress.total > 0}
                    ...
                    <p class="text-xs" style={`color:${isDark ? 'var(--o3-color-palette-white-80)' : 'var(--o3-color-palette-black-70)'}`}>
                ...
                {:else}
                    <div class="flex items-center gap-2">
                        <div class="animate-spin h-4 w-4 border-2 border-o3-teal border-t-transparent rounded-full"></div>
                        <p class="text-xs" style={`color:${isDark ? 'var(--o3-color-palette-white-80)' : 'var(--o3-color-palette-black-70)'}`}>Processing...</p>
                    </div>
                {/if}
            </div>
        {/if}
        ...
        <div class="space-y-6">
            ...
            <!-- OPML -->
            <div class="space-y-2">
                <h3 class="text-sm font-bold uppercase tracking-wider text-o3-teal">OPML Import/Export</h3>
                <div class="flex gap-2">
                    <button
