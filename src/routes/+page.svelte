...
import { themeMode, showSidebar, showArticleList } from "$lib/stores";
...
<div
    ...
    style={`background:${$themeMode === "dark" ? "var(--o3-color-palette-black-90)" : "var(--o3-color-palette-paper)"};grid-template-columns:${$showSidebar ? "280px " : ""}${$showArticleList ? "380px " : ""}1fr`}
>
    <!-- Left Panel: Navigation (280px fixed, toggleable with 'b' hotkey) -->
    {#if $showSidebar}
        <aside class="hidden md:block border-r border-o3-black-30 h-screen" style="width: 280px;">
            <Sidebar />
        </aside>
    {/if}

    <!-- Middle Panel: Feed (380px fixed, toggleable with 'z' hotkey) -->
    {#if $showArticleList}
        <section class="border-r border-o3-black-30 h-screen" style="width: 380px;">
            <ArticleList />
        </section>
    {/if}
    ...
    <main
        class="h-screen"
        style={`background:${$themeMode === "dark" ? "var(--o3-color-palette-black-90)" : "var(--o3-color-palette-white)"};`}
    >
        <Reader />
    </main>
</div>
